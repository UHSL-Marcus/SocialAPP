...
namespace WebApplication1.Views
{
    public partial class Stats : System.Web.UI.UserControl
    {
        ...
        {
            if ((Boolean)Session[Paths.INITIAL_LOAD])
            {
                fillTownList();
                Session[Paths.INITIAL_LOAD] = false;
            }
            ScriptManager.RegisterStartupScript(statsTownList, statsTownList.GetType(), "stats" + UniqueID, "statsVisualCommands();", true);
            ScriptManager.RegisterStartupScript(statsTownList, statsTownList.GetType(), "pageColour" + UniqueID, @"changeCurrentPage(""stats"");", true);
        ...
        private void displayTownInfo(String town)
        {
            //TODO: Try Catch
            if (town.Equals("Home"))
                town = (new XMLParse((String)Session[Paths.USERDETAILS], SOAPRequest.soapNamespace)).getElementText("Town");
            ...
            if (tID == returnTID)
            {
                ...
                Dictionary<string, int> categoryRating = new Dictionary<string, int>();
